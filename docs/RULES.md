...
| **Hot reactivation** — an already-Hot account re-fires when its newest touch is newer than the last-notified touch. Scope decision: applies to NEW activity only; the 2026-07-10 baselined backlog stays silenced (working backlog Hots is a campaign decision, not a notification) | Galyna 2026-07-03 · shipped 2026-07-05 · scope Sunny 2026-07-20 | same |
| **Non-ABM never activates** — only accounts on the ABM list get activation cards; unmatched inbound is dropped, not queued | Sunny · standing (2026-07-08) | `cross_and_persist(persist_unmatched=False)` + strict-ABM eval |
